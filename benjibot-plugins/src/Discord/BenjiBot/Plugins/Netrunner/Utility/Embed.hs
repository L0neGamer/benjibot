...
module Discord.BenjiBot.Plugins.Netrunner.Utility.Embed
  ( cardToEmbed,
    cardToEmbedWithText,
    cardsToEmbed,
    cardToImgEmbed,
...
import Discord.BenjiBot.Plugins.Netrunner.Utility.Card
import Discord.BenjiBot.Plugins.Netrunner.Utility.Faction
...
cardsToEmbed :: NrApi -> Text -> [Card] -> Text -> EnvDatabaseDiscord NrApi CreateEmbed
...
embedLines title pre xs =
  let cumLength = scanl (\l x -> 1 + T.length x + l) (T.length title + 2) xs -- +1 for each newline title characters
      ...
      c = if length xs' < 12 then 1 else 2
      d = length xs' `div` c
      m = length xs' `mod` c
