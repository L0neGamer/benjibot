-- |
-- Module      : Discord.BenjiBot.Plugins.Administration
...
module Discord.BenjiBot.Plugins.Administration (administrationPlugin) where
...
import Discord.BenjiBot.Internal.Cache (getVersionInfo)
import Discord.BenjiBot.Internal.Types (CompiledPlugin (compiledName))
...
version = Command "version" noCommand []
  where
    ...
    formatVersions vi = "Discord.BenjiBot version " <> pack (showVersion $ procVersion vi) <> "\nGit Hash: `" <> gitHash vi <> "`"
