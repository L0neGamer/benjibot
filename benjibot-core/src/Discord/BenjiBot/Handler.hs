-- |
-- Module      : Discord.BenjiBot.Handler
-- Description : The event handler and cron runner for Discord.BenjiBot.
-- License     : MIT
-- Maintainer  : tagarople@gmail.com
-- Stability   : experimental
-- Portability : POSIX
--
-- This module provides most of the functionality in "Discord.BenjiBot". This
...
import Discord.BenjiBot.Internal.Handler.Command (parseNewMessage)
import Discord.BenjiBot.Internal.Handler.Event
...
import Discord.BenjiBot.Utility.Discord (createApplicationCommand, interactionResponseCustomMessage, removeApplicationCommandsNotInList, sendChannelEmbedMessage)
import Discord.BenjiBot.Utility.Exception (BotException, embedError)
import Discord.BenjiBot.Utility.Types (MessageDetails (messageDetailsEmbeds), BenjiBotCache (cacheApplicationCommands), liftDiscord, messageDetailsBasic)
...
runCron ::
  Pool SqlBackend ->
  CompiledCronJob ->
  ReaderT (MVar BenjiBotCache) DiscordHandler ThreadId
runCron pool (CCronJob delay fn) = do
  cache <- ask
  lift . forkIO $ withDelay cache
  where
    withDelay :: MVar BenjiBotCache -> DiscordHandler ()
    withDelay cache = do
